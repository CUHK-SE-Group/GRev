...
class HugeGraphTester(TesterAbs):
    def __init__(self, database):
        self.database = database

    def single_file_testing(self, logfile):
        logger = new_logger("logs/hugegraph.log")
...
def schedule():
    scheduler(config.get('hugegraph', 'input_path'), HugeGraphTester(f"hugegraph"), "hugegraph")


if __name__ == "__main__":
    if config.get('GLOBAL', 'env') == "debug":
        Tester = HugeGraphTester('tinkerpop')
